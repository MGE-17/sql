...
-- Step 1: Number with most recent = 1
WITH
    visits AS (
        SELECT
            customer_id,
    ...
    -- Step 2: Filter to only most recent visit
SELECT *
FROM visits
WHERE
    visit_number = 1;
...
WITH
    vp AS ( -- vendor × product pairs with a price
    ...
    cust AS ( -- total # of customers
        SELECT COUNT(*) AS num_customers
        FROM customer
    )
SELECT vp.vendor_name, vp.product_name, vp.original_price, (
        cust.num_customers * 5 * vp.original_price
