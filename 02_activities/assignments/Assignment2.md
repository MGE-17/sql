...
The store can set up the CUSTOMER_ADDRESS table in two different ways:
Option 1 – Overwrite (Type 1)

Each customer only has one row for their current address.

If the address changes, the old one is replaced by the new one.

This is Type 1 SCD.

Option 2 – Keep history (Type 2)

Each time a customer’s address changes, a new row is added.

The table can have many rows for the same customer (one for each old address).

We add columns like start_date, end_date, or a flag to mark the current address.
