...
SELECT v.vendor_id, v.vendor_name, COUNT(vba.vendor_id) AS boot_rental_count
FROM
    vendor AS v
    INNER JOIN vendor_booth_assignments AS vba ON v.vendor_id = vba.vendor_id
GROUP BY
...
SELECT c.customer_id, c.customer_first_name, c.customer_last_name, SUM(
        cp.cost_to_customer_per_qty * cp.quantity
    ) AS total_spent
FROM
    customer AS c
